...
if debug:
    for (topic_id, wordprob) in sorted(topic_wordprob.items()):
        output_dat.write("topic#"+str(topic_id)+"\n")
        for (word, prob) in sorted(wordprob.items(), key=operator.itemgetter(1), reverse=True):
            output_dat.write("\t"+word + "|||" + ("%.10f" % prob)+"\n")
